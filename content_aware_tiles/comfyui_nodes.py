...
from .generation import unpack_tiles, pack_tiles,  random_tiling, slide_wrapping, unslide_wrapping, call_chunked, inpainting_from_boundaries, random_cuts, select_candidates
...
class RandomTiling:
    ...
    def generate(self, tiles: torch.Tensor, tileset: Tileset, width: int, height: int):
        ...
        if tileset.kind == 'dual' and x.shape[0] != tileset.colors**4*2:
            raise ValueError(f'must generate interior and cross tiles ({tileset.colors**4*2} total), found {x.shape[0]} tiles')
        x = random_tiling(x, tileset.colors, tileset.kind, indices=(1, height, width))
        x = x.permute(0, 2, 3, 1)
        return [x]
